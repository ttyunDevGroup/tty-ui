...
export default {
  ...
  mounted() {
    if (this.prop) {
      this.dispatch('TForm', 't.form.addField', [this])
      ...
      let rules = this.getRules()
      if (rules.length) {
        this.$on('t.form.blur', this.onFieldBlur)
        this.$on('t.form.change', this.onFieldChange)
      }
    }
  },
  beforeDestroy() {
    this.dispatch('BForm', 't.form.removeField', [this])
  }
}
</script>