<template>
  <t-input class="t-date-editor" :class="'t-date-editor--' + type" :readonly="readonly" :disabled="disabled" :size="size" v-clickoutside="handleClose" :placeholder="placeholder" @focus="handleFocus" @blur="handleBlur" @keydown.native="handleKeydown" :value="displayValue" @change.native="displayValue = $event.target.value" :validateEvent="false" ref="reference">
    <i slot="icon" class="t-input__icon input-group-inner-addon" @click="handleClickIcon" :class="[showClose ? 't-icon-close' : triggerClass]" @mouseenter="handleMouseEnterIcon" @mouseleave="showClose = false" v-if="haveTrigger"></i>
  </t-input>
</template>
...
<script>
  ...
  export default {
    ...
    watch: {
      pickerVisible(val) {
        if(!val) this.dispatch('TFormItem', 't.form.blur')
      ...
      displayValue(val) {
        ...
        this.dispatch('TFormItem', 't.form.change')
      }
    },
    computed: {
      reference() {
      ...
      triggerClass() {
        return this.type.indexOf('time') !== -1 ? 't-icon-time' : 't-icon-calendar'
      },
      selectionMode() {
        if(this.type === 'week') {
          return 'week'
