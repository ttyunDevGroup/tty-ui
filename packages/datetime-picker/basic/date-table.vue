<template>
  <table cellspacing="0" cellpadding="0" class="t-date-table" @click="handleClick" @mousemove="handleMouseMove" :class="{ 'is-week-mode': selectionMode === 'week' }">
    <tbody>
      <tr>
        <th v-if="showWeekNumber">周次</th>
        <th v-for="week in WEEKS">{{week}}</th>
      </tr>
      <tr class="t-date-table__row" v-for="row in rows" :class="{ current: isWeekActive(row[1]) }">
